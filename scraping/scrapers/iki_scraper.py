...
class IkiScraper:
    ...
    def extract_containers_of_products_w_pricetags(self, soup):
        ...
        for product_container in product_containers:
            if product_container.find('div', class_='price_int'):
    ...
    def extract_product_name(self, product_container):
        product_element = product_container.find('p', class_='akcija_title w-100')
        product_name = product_element.text.strip().lower()
        return product_name

    def extract_product_price(self, product_container):
        price_whole_element = product_container.find('div', class_='price_int')
        price_decimal_element = product_container.find('span', class_='sub')
    ...
    def get_iki_product_list(self, product_containers):
        mapping = dict()
        for product_container in product_containers:
